...
self.addEventListener('fetch', e => {
  ...
  if (url.pathname.endsWith('data.json') || url.pathname.endsWith('bearing.json')) {
    const file = url.pathname.endsWith('bearing.json') ? './bearing.json' : './data.json';
    e.respondWith(
      fetch(req).then(r => {
        const cp = r.clone();
